...
static void TXT_ScrollPaneDrawer(TXT_UNCAST_ARG(scrollpane), int selected)
{
	...
	x1 = scrollpane->widget.x, y1 = scrollpane->widget.y, x2 = x1 + scrollpane->w, y2 = y1 + scrollpane->h;

	scrollbars = NeedsScrollbars(scrollpane);

	if (scrollbars & SCROLLBAR_HORIZONTAL)
	{
		TXT_DrawHorizScrollbar(x1, y1 + scrollpane->h, scrollpane->w, scrollpane->x, FullWidth(scrollpane) - scrollpane->w);
	}

	if (scrollbars & SCROLLBAR_VERTICAL)
	{
		TXT_DrawVertScrollbar(x1 + scrollpane->w, y1, scrollpane->h, scrollpane->y, FullHeight(scrollpane) - scrollpane->h);
	}
...
static void ShowSelectedWidget(txt_scrollpane_t * scrollpane)
{
	...
	{
		scrollpane->y += (selected->y + selected->h) - (scrollpane->widget.y + scrollpane->h);
	}
	...
	{
		scrollpane->x += (selected->x + selected->w) - (scrollpane->widget.x + scrollpane->w);
	}
}
...
static int TXT_ScrollPaneKeyPress(TXT_UNCAST_ARG(scrollpane), int key)
{
	...
	if (scrollpane->child != NULL)
	{
		...
		if (scrollpane->child->widget_class == &txt_table_class
			&& (key == KEY_UPARROW || key == KEY_DOWNARROW || key == KEY_LEFTARROW || key == KEY_RIGHTARROW || key == KEY_PGUP || key == KEY_PGDN))
		{
...
static void TXT_ScrollPaneMousePress(TXT_UNCAST_ARG(scrollpane), int x, int y, int b)
{
	TXT_CAST_ARG(txt_scrollpane_t, scrollpane);
	int scrollbars;
