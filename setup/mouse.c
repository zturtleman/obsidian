...
static void MouseSetCallback(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(variable))
{
	...
	{
		if (*all_mouse_buttons[i] == *variable && all_mouse_buttons[i] != variable)
		{
...
static void ConfigExtraButtons(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
	...
	TXT_AddWidgets(window, buttons_table = TXT_NewTable(2), NULL);
...
void ConfigMouse(void)
{
	...
	TXT_AddWidgets(window,
				   ...
				   TXT_NewSeparator("Buttons"), buttons_table = TXT_NewTable(2), more_buttons = TXT_NewButton("More buttons..."), NULL);
	...
	TXT_AddWidgets(motion_table,
				   ...
				   TXT_NewLabel("Acceleration threshold"), TXT_NewSpinControl(&mouse_threshold, 0, 32), NULL);
