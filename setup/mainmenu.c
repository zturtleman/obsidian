...
static void QuitConfirm(void *unused1, void *unused2)
{
	...
	TXT_AddWidgets(window,
				   ...
				   TXT_NewStrut(24, 0), yes_button = TXT_NewButton2("  Yes  ", DoQuit, DoQuit), no_button = TXT_NewButton2("  No   ", DoQuit, NULL), NULL);
...
void MainMenu(void)
{
	...
	TXT_AddWidgets(window,
				   ...
				   TXT_NewButton2("Multiplayer Configuration", (TxtWidgetSignalFunc) MultiplayerConfig, NULL), NULL);
...
static void SetIcon(void)
{
	...
	{
		if (setup_icon_data[i * 3] != 0x00 || setup_icon_data[i * 3 + 1] != 0x00 || setup_icon_data[i * 3 + 2] != 0x00)
		{
			mask[i / 8] |= 1 << (7 - i % 8);
		}
	}

	surface = SDL_CreateRGBSurfaceFrom(setup_icon_data, setup_icon_w, setup_icon_h, 24, setup_icon_w * 3, 0xff << 0, 0xff << 8, 0xff << 16, 0);
