...
static void SetJoystickButtonLabel(void)
{
	...
	if (joystick_initted && joystick_index >= 0 && joystick_index < SDL_NumJoysticks())
	{
...
{
	if (event->type == SDL_JOYBUTTONDOWN && (joystick_index == -1 || event->jbutton.which == joystick_index))
	{
		switch (calibrate_stage)
		{
			case CALIBRATE_CENTER:
...
static void NoJoystick(void)
{
	...
	TXT_AddWidget(window, TXT_NewLabel("No joysticks could be opened.\n\n" "Try configuring your joystick from within\n" "your OS first."));
...
static void CalibrateJoystick(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
	...
	TXT_AddWidgets(calibration_window,
				   TXT_NewLabel("Please follow the following instructions\n"
								"in order to calibrate your joystick."), TXT_NewStrut(0, 1), calibration_label = TXT_NewLabel("zzz"), TXT_NewStrut(0, 1), NULL);
...
void JoyButtonSetCallback(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(variable))
{
	...
	{
		if (variable != all_joystick_buttons[i] && *variable == *all_joystick_buttons[i])
		{
...
void ConfigJoystick(void)
{
	...
	TXT_AddWidgets(window,
				   ...
				   joystick_table = TXT_NewTable(2), TXT_NewSeparator("Joystick buttons"), button_table = TXT_NewTable(2), NULL);

	TXT_SetColumnWidths(joystick_table, 20, 15);

	TXT_AddWidgets(joystick_table, TXT_NewLabel("Current joystick"), joystick_button = TXT_NewButton("zzzz"), NULL);
