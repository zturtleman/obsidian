...
static Channel *Channel__BlockTemplate(Channel * self, Chip * chip, Bit32u samples, Bit32s * output, SynthMode mode);
...
BLOCK_TEMPLATE(sm3FMFM) BLOCK_TEMPLATE(sm3AMFM) BLOCK_TEMPLATE(sm3FMAM) BLOCK_TEMPLATE(sm3AMAM) BLOCK_TEMPLATE(sm2Percussion) BLOCK_TEMPLATE(sm3Percussion)
...
TEMPLATE_VOLUME(OFF) TEMPLATE_VOLUME(RELEASE) TEMPLATE_VOLUME(SUSTAIN) TEMPLATE_VOLUME(ATTACK) TEMPLATE_VOLUME(DECAY)
...
static inline void Channel__GeneratePercussion(Channel * self, Chip * chip, Bit32s * output, int opl3Mode)
{
	Channel *chan = self;
...
Channel *Channel__BlockTemplate(Channel * self, Chip * chip, Bit32u samples, Bit32s * output, SynthMode mode)
{
	Bitu i;
	...
	switch (mode)
	{
		...
		case sm3FMAM:
			...
			break;
		case sm3AMAM:
			if (Operator__Silent(Channel__Op(self, 0)) && Operator__Silent(Channel__Op(self, 2)) && Operator__Silent(Channel__Op(self, 3)))
			{
				self->old[0] = self->old[1] = 0;
				return (self + 2);
			}
