...
static void CheckTargetTimeouts(void)
{
	...
	for (i = 0; i < num_targets; ++i)
	{
		...
		if (targets[i].state == QUERY_TARGET_QUERIED
			&& targets[i].query_attempts >= QUERY_MAX_ATTEMPTS && now - targets[i].query_time > QUERY_TIMEOUT_SECS * 1000)
		{
...
void NET_QueryPrintCallback(net_addr_t * addr, net_querydata_t * data, unsigned int ping_time, void *user_data)
{
	...
	formatted_printf(8, "%i/%i", data->num_players, data->max_players);
