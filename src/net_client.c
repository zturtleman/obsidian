...
static void NET_CL_AdvanceWindow(void)
{
	while(recvwindow[0].active)
	{
		...
		memcpy(recvwindow, recvwindow + 1, sizeof(net_server_recv_t) * (BACKUPTICS - 1));
...
static void NET_CL_ParseWaitingData(net_packet_t * packet)
{
	...
	if (!NET_ReadInt8(packet, &num_players)
		|| !NET_ReadInt8(packet, &num_drones) || !NET_ReadInt8(packet, &is_controller) || !NET_ReadSInt8(packet, &player_number))
	{
		// invalid packet
	...
	if ((player_number >= 0 && drone) || (player_number < 0 && !drone) || (player_number >= (signed int)num_players))
	{
	...
	if (!NET_ReadMD5Sum(packet, wad_md5sum) || !NET_ReadMD5Sum(packet, deh_md5sum) || !NET_ReadInt8(packet, &server_is_freedoom))
	{
		return;
	}
...
static void NET_CL_ParseGameStart(net_packet_t * packet)
{
	...
	if (!NET_ReadInt8(packet, &num_players) || !NET_ReadSInt8(packet, &player_number) || !NET_ReadSettings(packet, &settings))
	{
		return;
	}
	...
	if ((drone && player_number >= 0) || (!drone && player_number < 0))
	{
...
void NET_CL_Run(void)
{
	...
	net_waiting_for_start = client_connection.state == NET_CONN_STATE_CONNECTED && client_state == CLIENT_STATE_WAITING_START;
