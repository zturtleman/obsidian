...
int remove(const char *pathname)
{
	...
	MultiByteToWideChar(CP_OEMCP, 0, pathname, strlen(pathname) + 1, temp, MAX_PATH);
...
int rename(const char *oldpath, const char *newpath)
{
	...
	MultiByteToWideChar(CP_OEMCP, 0, oldpath, strlen(oldpath) + 1, oldpath1, MAX_PATH);
	MultiByteToWideChar(CP_OEMCP, 0, newpath, strlen(newpath) + 1, newpath1, MAX_PATH);
